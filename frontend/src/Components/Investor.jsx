...
const BrowseInvestors = () => {
  return (
    <section id="investors" className="w-full bg-gradient-to-b from-[#f8faff] via-white to-[#f8faff] py-12 md:py-20 lg:py-24 2xl:py-16 overflow-hidden">
      <div className="w-full max-w-[85rem] 2xl:max-w-6xl mx-auto px-6 sm:px-12 lg:px-20 grid grid-cols-1 lg:grid-cols-2 gap-12 sm:gap-16 lg:gap-20 2xl:gap-16 items-center">
        ...
        <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ amount: 0.2 }} transition={{ duration: 0.8 }} className="order-last lg:order-first">
          ...
          <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-[3rem] xl:text-[3.5rem] 2xl:text-[2.5rem] font-bold leading-[1.15] tracking-tight text-slate-900 mb-8 sm:mb-10">
            Fuel the Revolution. <br />
            <span className="text-indigo-600 italic font-serif pr-2">Invest in Global Greatness</span>
          </h2>

          <p className="text-sm sm:text-base md:text-lg lg:text-lg xl:text-xl 2xl:text-base font-medium leading-relaxed text-slate-900 mb-12 max-w-lg lg:max-w-xl 2xl:max-w-lg">
          ...
          <div className="space-y-8 sm:space-y-10 2xl:space-y-8">
            {[
              ...
              <motion.div 
                ...
                className="flex items-center gap-8 2xl:gap-6 group"
              >
                <div className={`flex-shrink-0 w-14 h-14 2xl:w-12 2xl:h-12 rounded-2xl ${feature.color} flex items-center justify-center transition-all bg-opacity-70 shadow-sm group-hover:scale-105`}>
                  {React.cloneElement(feature.icon, { className: "w-7 h-7 2xl:w-6 2xl:h-6" })}
                </div>
                <div>
                  <h4 className="font-bold text-slate-900 text-lg 2xl:text-base mb-1.5 2xl:mb-1">{feature.title}</h4>
                  <p className="text-sm sm:text-base lg:text-base xl:text-lg 2xl:text-sm font-medium text-slate-600 max-w-sm">{feature.desc}</p>
                </div>
              </motion.div>
            ))}
          </div>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95, x: 50 }} whileInView={{ opacity: 1, scale: 1, x: 0 }} viewport={{ amount: 0.2 }} transition={{ duration: 0.8 }} className="flex justify-center">
          <div className="relative w-full max-w-[300px] sm:max-w-[440px] lg:max-w-[480px] xl:max-w-xl 2xl:max-w-[420px]">
            <div className="absolute -inset-10 bg-gradient-to-r from-indigo-500/10 to-purple-500/10 rounded-3xl blur-3xl opacity-40 shadow-2xl"></div>
