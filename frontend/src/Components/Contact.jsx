...
const ContactUs = () => {
  ...
  return (
    <section id="contact" className="relative py-12 md:py-20 lg:py-24 2xl:py-20 bg-gradient-to-b from-[#f8faff] via-white to-[#f3f6ff] overflow-hidden">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">
          ...
          <div className="space-y-8">
            {[
              ...
              <a key={index} href={item.link} className="flex items-center gap-6 bg-white rounded-2xl p-6 sm:p-8 shadow-sm border border-slate-100 hover:shadow-2xl transition-all duration-300">
                <div className="flex-shrink-0 h-12 w-12 sm:h-14 sm:w-14 flex items-center justify-center rounded-2xl bg-indigo-600 text-white shadow-lg">
                  {item.icon}
                </div>
                <div>
                  <h4 className="text-base sm:text-lg font-bold text-slate-900 mb-1">{item.title}</h4>
                  <p className="text-slate-600 text-sm sm:text-base font-medium">{item.value}</p>
                </div>
              </a>
            ))}
          </div>
          ...
          {/* Form */}
          <div className="bg-white rounded-[3rem] shadow-2xl p-8 sm:p-12 border border-slate-100">
            <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-8">
                <div>
                  <label className="block text-xs font-bold text-slate-900 mb-3 uppercase">Full Name</label>
                  <input type="text" name="name" value={form.name} onChange={handleChange} onBlur={handleBlur} placeholder="Name" className="w-full px-6 py-4 rounded-xl border-2 border-slate-100 bg-slate-50 focus:border-indigo-600 focus:outline-none transition-all text-base font-medium" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-900 mb-3 uppercase">Email</label>
                  <input type="email" name="email" value={form.email} onChange={handleChange} onBlur={handleBlur} placeholder="Email" className="w-full px-6 py-4 rounded-xl border-2 border-slate-100 bg-slate-50 focus:border-indigo-600 focus:outline-none transition-all text-base font-medium" />
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-8">
                <div>
                  <label className="block text-xs font-bold text-slate-900 mb-3 uppercase">Phone</label>
                  <input type="tel" name="mobile" value={form.mobile} onChange={handleChange} onBlur={handleBlur} placeholder="Phone" className="w-full px-6 py-4 rounded-xl border-2 border-slate-100 bg-slate-50 focus:border-indigo-600 focus:outline-none transition-all text-base font-medium" />
                </div>
                <div className="relative" ref={dropdownRef}>
                  <label className="block text-xs font-bold text-slate-900 mb-3 uppercase">Address</label>
                  <input type="text" name="address" value={addrQuery} onChange={handleChange} onBlur={handleBlur} placeholder="Address" className="w-full px-6 py-4 rounded-xl border-2 border-slate-100 bg-slate-50 focus:border-indigo-600 focus:outline-none transition-all text-base font-medium" />
              ...
              <div>
                <label className="block text-xs font-bold text-slate-900 mb-3 uppercase">Message</label>
                <textarea name="message" rows="4" value={form.message} onChange={handleChange} placeholder="Message" className="w-full px-6 py-4 rounded-xl border-2 border-slate-100 bg-slate-50 focus:border-indigo-600 focus:outline-none transition-all resize-none text-base font-medium" />
              </div>
              <button type="submit" disabled={isLoading} className="w-full py-5 bg-indigo-600 text-white font-bold rounded-2xl shadow-xl hover:bg-indigo-700 transition-all text-lg active:scale-[0.98]">
