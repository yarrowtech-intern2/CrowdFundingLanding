    ...
    >
      <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-[#1e2b4f] mb-5">
            Security &amp;{" "}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-8">
          {items.map((item, index) => (
            <motion.div
              ...
              className={`bg-white rounded-2xl sm:rounded-3xl p-6 sm:p-8 shadow-lg border border-gray-100 hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 flex flex-col group ${index === 2 ? "sm:col-span-2 lg:col-span-1" : ""}`}
            >
              <div className="h-12 w-12 rounded-2xl bg-[#2e66ff] flex items-center justify-center shadow-lg group-hover:bg-[#1e2b4f] transition-colors duration-300 mb-5 sm:mb-6">
                {item.icon}
              </div>

              <h3 className="text-base sm:text-lg font-semibold text-[#1e2b4f] mb-3 sm:mb-4">
                {item.title}
              </h3>
