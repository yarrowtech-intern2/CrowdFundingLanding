...
const SecurityCompliance = () => {
  const items = [
    {
      ...
      desc: "All transactions and personal data are encrypted with 256-bit SSL, ensuring your financial information remains completely private and protected.",
      icon: <MdLock className="w-6 h-6" />,
    },
    {
      id: 2,
      title: "Regulatory Compliance",
      desc: "Fully compliant with SEBI and government regulations. Our platform undergoes regular audits to ensure total adherence to Laws.",
  ...
  return (
    <section id="security" className="w-full bg-[#fdfdff] py-12 md:py-20 lg:py-24 2xl:py-20 overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 sm:gap-10">
          {items.map((item, index) => (
            <motion.div 
              ...
              className="bg-white rounded-[2.5rem] p-10 sm:p-12 shadow-sm border border-slate-100 hover:bg-indigo-50/20 hover:border-indigo-100 hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 flex flex-col group cursor-default"
            >
              <div className="w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg mb-10 bg-indigo-600 text-white group-hover:scale-110 transition-transform">
                {item.icon}
              </div>
              <h3 className="text-xl sm:text-2xl font-bold text-slate-900 mb-6 tracking-tight group-hover:text-indigo-600 transition-colors">{item.title}</h3>
